...
According to https://www.w3.org/TR/webarch/#representation-management[W3C], "a URI owner SHOULD provide representations of the resource it identifies". 

We use the services provided by w3id.org, and define https://github.com/perma-id/w3id.org/tree/master/demo-resources[RewriteRules for dereferencing our URIs, window="_blank", role="ext-link"]. 

We also use the services provided by w3id.org to implement content negotiation  (incl. language negotiation), such that the same URI may return RDF Turtle or HTML in the preferred language, depending on "who's asking".

For example, for the URI `\https://w3id.org/demo-resources/demo-concepts#demoCpt1`: 

* `curl -i -H "Accept: text/turtle" \https://w3id.org/demo-resources/demo-concepts#demoCpt1` will redirect to the RDF Turtle representation of the resource identified by that URI.

* `curl -i -H "Accept-language: nb" \https://w3id.org/demo-resources/demo-concepts#demoCpt1` will redirect to the HTML representation, in Norwegian bokmål, of the resources identified by that URI. 

=== Why is a permanent URI important?

The answer I got when I (2023-03-30) asked ChatGPT the above question, was as follows: 
