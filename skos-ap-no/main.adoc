...
* An information model may refer to concepts that are relevant for understanding the information model, by using the property https://data.norge.no/specification/modelldcat-ap-no#Egenskap-begrep[subject (dct:subject) ↗,  window="_blank", role="ext-link"].
