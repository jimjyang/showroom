...
** In general, an information model itself, its model elements (object types, code lists etc.) and properties (attributes, associations etc.) may all refer to concepts, using the property `subject (skos:Concept)` in accordance with ModellDCAT-AP-NO.  
